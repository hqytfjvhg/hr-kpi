<template>
  <div>
    ...
    <div class="dynysjcontent" v-if="isShowKpiTable">
      ...
      >
        <tr class="title" style="border: none; font-size: 20px">
          <th colspan="10">
            <img src="../../assets/kyLogo.png" alt="" style="position: absolute; top: 2px; left: 2px; width: 5rem" />
        ...
        <tr style="height: 1.5rem; font-size: 0.8rem; border: 1px solid black">
      ...
      <table border="1" color="black" align="center" cellspacing="0" cellpadding="0" class="table2-style">
        <tr class="title" style="border: none; font-size: 1rem">
          <th colspan="10">
            <img src="../../assets/kyLogo.png" alt="" style="position: absolute; top: 0.1rem; left: 2px; width: 4rem" />
            <span> 东莞凯韵科技有限公司<br />个人绩效月考评表</span>
          </th>
        </tr>
        <tr
        ...
        <tr style="height: 1.5rem; font-size: 0.7rem; border: 1px solid black">
          <td colspan="2">{{ previewKpiHistoryData.name }}</td>
          <td colspan="2">{{ previewKpiHistoryData.deptName }}</td>
          <td colspan="2">{{ previewKpiHistoryData.position }}</td>
          <td colspan="1">{{ previewKpiHistoryData.valueInfoList[0].month }}月</td>
        ...
        <tr
          v-for="(item, index) in previewKpiHistoryData.targetInfoList"
          :key="index"
          style="height: 1.5rem; font-size: 0.7rem; border: 1px solid black"
        >
          <td>{{ index + 1 }}</td>
          <td>{{ item.targetName }}</td>
          <td>{{ item.weight }}</td>
        ...
        <tr class="bold" style="height: 1.5rem; font-size: 0.8rem; font-weight: 700; border: 1px solid black">
          <th colspan="2">业绩实际得分</th>
          <th colspan="8">{{ previewKpiHistoryData.performanceActualScore }}</th>
        </tr>
        <tr class="bold" style="height: 1.5rem; font-size: 0.8rem; font-weight: 700; border: 1px solid black">
    ...
    <div style="text-align: right; padding: 0.5rem" v-if="isShowKpiTable">
      <van-button @click="reView" style="padding: 0 1rem" size="small">预览</van-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    //提交签名
    onSubmit(data) {
      showConfirmDialog({ message: "是否确认提交签名" }).then(() => {
