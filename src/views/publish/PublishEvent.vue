  ...
  <div class="model">
    <div class="button-group">
      <div class="form-style">
        <el-form class="form">
          <el-form-item label="名称" width="100">
            <el-input v-model="selectEventName" :placeholder="$t('input')" />
          </el-form-item>

          <el-form-item label="是否使用">
            <el-select clearable :placeholder="$t('choose')" v-model="useState">
              <el-option
          ...
          <el-button @click="selectEventList" type="primary">{{ $t("inquire") }}</el-button>
        </el-form>
      </div>
      ...
      <el-button type="warning" @click="createEvent">
        <el-icon class="iconStyle"><Position /></el-icon>{{ $t("release") }}
      </el-button>
      <el-tooltip content="查看参与人对比名单" placement="top" effect="light" v-if="surplusNameList.length > 0">
        <el-button type="info" @click="isPublish = true">
          <el-icon class="iconStyle"><calendar /></el-icon>{{ $t("check") }}
        </el-button>
      </el-tooltip>
      <el-button type="danger" @click="deleteEvent()">
        <el-icon class="iconStyle"><Delete /></el-icon>{{ $t("delete") }}
      </el-button>
      ...
      <el-tooltip placement="top" effect="light" width="300">
        ...
        >
        <el-button type="primary" @click="downloadLastMonthExcel"
          ><el-icon><Download /></el-icon>{{ $t("lastTemplet") }}</el-button
        >
      </el-tooltip>

      <el-tooltip content="下载的文件中员工考核指标规则（第一张工作簿）是空" placement="top" effect="light">
        <el-button type="primary" @click="downloadExcel"
          ><el-icon><Download /></el-icon>{{ $t("templet") }}</el-button
        >
      </el-tooltip>
    </div>
    <el-table
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <div>
            ...
            <el-button type="primary" size="small" circle @click="setEventData(scope.row)" v-if="!scope.row.state"
              ><el-icon><Tools /></el-icon
            ></el-button>
            <el-button
              type="danger"
              size="small"
              circle
              @click="backEvent(scope.row)"
              v-if="!scope.row.eventKpiDataSaveState"
    ...
    <!-- 选择模板创建事件 -->
    <el-dialog v-model="isModel" title="请为事件选择模板" :show-close="false" :close-on-click-modal="false">
      <el-table
        border
        stripe
        :data="modelTableData"
    ...
    <!-- 预览文件的弹窗 -->
    <el-dialog v-model="isExcelShow" title="预览文件" :eventId="eventData.eventId">
      <el-table border height="50vh" v-if="targetListData[eventData.eventId]" :data="targetListData[eventData.eventId]">
      ...
      <div class="button-style">
        <el-button @click="isExcelShow = false" type="primary">确定</el-button>
      </div>
    </el-dialog>
    <!-- 业绩设置参数的弹窗 -->
    <el-dialog
      v-model="isTargetShow"
      title="业绩审批流程"
      style="min-height: 600px"
      align-center
      :close-on-click-modal="false"
    >
      <div class="reuser-button">
        <el-button type="primary" size="small" @click="getLastEventFlow">一键复用</el-button>
      </div>
      <el-table :data="deptNameList" style="height: 50vh" row-style="height:60px" border>
        <el-table-column label="名称" prop="deptName"></el-table-column>
        <el-table-column label="选择审批流程">
          <template #default="scope">
            <el-select v-model="scope.row.performanceFlowId" placeholder="请选择" size="small">
              <el-option
        ...
        <el-table-column label="业绩审批流程">
          <template #default="scope">
            <el-timeline class="timelineStyle" v-if="scope.row.performanceFlowId">
              <el-timeline-item
                class="linitemStyle active"
                type="primary"
                placement="center"
                v-for="item in getSelectedUsers(scope.row.performanceFlowId)"
                :key="item"
              >
                {{ item }}
              </el-timeline-item>
      ...
      <div class="button-style">
        ...
        <el-button @click="selectPerformance" type="primary">确定</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      align-center
    >
      <div v-if="weightData.length > 0" class="error-tooltip-one">
      ...
      <div v-if="nameList.length > 0 || userSelectName.length > 0" class="error-tooltip-two">
        请重新选择价值观参与人或修改excel文件,价值观考核参与人与业绩考核参与人必须一致，否则发布失败。
      </div>
      <div class="error-table" v-if="nameList.length > 0 || userSelectName.length > 0">
      ...
      <div v-if="surplusNameList.length > 0" class="error-tooltip-three">
        以下人员未参与本次事件任务考核，请确认名单，人员不影响本次发布。
      </div>
      <div class="error-table" v-if="surplusNameList.length > 0">
      ...
      <div class="error-button">
        <el-button @click="handelClose" type="primary">确定</el-button>
      </div>
    </el-dialog>
    <!-- 权重不为100的人 -->
    <el-dialog v-model="isWeightEqual" title="权重总和数据不匹配名单" align-center>
      <div class="weight-tooltip">单个人员配置的指标权重之和必须为100%，请修改Excel文件重新上传。</div>
      <el-table :data="weightData" height="46vh" border>
        <el-table-column type="index" label="序号" width="80"></el-table-column>
        <el-table-column label="姓名" prop="0"></el-table-column>
        <el-table-column label="考核指标权重之和（%）" prop="weight"></el-table-column>
      </el-table>
      <div class="button-style">
        <el-button @click="isWeightEqual = false" type="primary">确定</el-button>
      </div>
    </el-dialog>
    ...
    >
      <el-timeline style="text-align: left">
        ...
        >
          {{ activity.content }}
          <el-card v-if="index == 0" class="card-style">
            <el-table border v-if="selectModelList[eventData.eventId]" :data="selectModelList[eventData.eventId].list">
              ...
              <el-table-column label="操作">
                <template #default="scope">
                  <el-button
                    ...
                    v-if="!scope.row.currentTimePublishState"
                  >
                    设置
                  </el-button>
                </template>
              ...
              <el-table-column label="状态">
                <template #default="scope">
                  <span style="color: #409eff" v-if="scope.row.completeSetup == true">已完成</span>
                </template>
              </el-table-column>
            </el-table>
          </el-card>
          <el-card v-if="index == 1" class="card-style">
            ...
            <el-table :data="targetName[eventData.eventId]" border v-if="targetName[eventData.eventId]">
              ...
              <el-table-column label="状态">
                <span style="color: #409eff" v-if="deptNameList.every((item) => item.performanceFlowId != null)">
                  已完成
                </span>
              </el-table-column>
            </el-table>
      ...
      <div class="button-style">
        ...
        ><el-button @click="saveModelSetInfo" type="primary">保存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
// import ApprovalView from "@/views/values/ApprovalDialog.vue";
import ApprovalView from "@/views/publish/EventDialog.vue";
import { ElMessage, ElMessageBox, ElTooltip } from "element-plus";
import { getPerformanceFlow, downloadExcel, downloadLastMonthExcel, backEvent } from "@/api/performance/index";
import { getLastEventFlow } from "@/api/userlist/index";
import {
  Calendar,
  Delete,
  Plus,
...
export default {
  ...
  data() {
    return {
      ...
      dialogApproval: false, //控制价值观审批设置按钮
  ...
  methods: {
    ...
    selectEventList() {
      if (this.selectEventName && this.useState != null && this.useState.toString().length > 0) {
        this.selectEventTableData = this.eventTableData.filter(
          (item) => item.state == this.useState && item.eventName.indexOf(this.selectEventName) >= 0,
        );
      } else if (this.selectEventName) {
        this.selectEventTableData = this.eventTableData.filter(
          (item) => item.eventName.indexOf(this.selectEventName) >= 0,
        );
    ...
    setEventData(row) {
      ...
      if (dataPerformance != undefined) {
        ...
        console.log(this.deptNameList, "业绩审批数据");
    ...
    approvalData(val) {
      ...
      this.tempList.filter((item) => {
        if (item.eventId == this.modelData.eventId) {
          ...
          const allDefined = item.deptAndHrFlowContents.every(
            (item1) => item1.deptFlowId !== undefined && item1.hrFlowId !== undefined,
          );
          if (allDefined) {
            data[0].completeSetup = true;
          } else {
            data[0].completeSetup = false;
          }
        }
      });
      ...
      const tempDataList = this.tempList.map((item) => {
        return {
          ...
          deptAndHrFlowContents: item.deptAndHrFlowContents,
        };
      });
      ...
      const data = {
        ...
        deptFlowIdList: this.tempList[0].uniqueDeptFlowIds,
        hrFlowIdList: this.tempList[0].uniqueHrFlowIds,
      };
        ...
        reader.onload = (ev) => {
          try {
            ...
            let useridList = excelData[0].map((item) => {
              ...
              // 确保idList不是undefined，才读取idList[5]
              if (idList && idList.length > 0) {
                // 只有当idList[5]是true时，才使用idList[1]作为部门名称
                deptName = idList[5] ? idList[1] : idList[3];
                ...
                return Object.assign(
                  ...
                  { weight: item[3] },
                  { customFlowState: idList ? idList[5] : null },
                  // { targetCustomFlowState: idList ? idList[5] : null },
                );
              }
            });
            ...
            // console.log(this.targetListData, "指标列表的数据");
            let uniqueDeptNames = this.targetListData[this.eventData.eventId]
            ...
            //自定义审批人员
            let customFlowList = this.targetListData[this.eventData.eventId]
              .filter((item) => item.customFlowState)
              .map((item) => ({
                deptName: item[0],
              }));
            // console.log(customFlowList);
            //业绩审批的部门，去重后
            ...
            //自定义审批人员去重
            // console.log([...new Set(customFlowList.map((item) => item.name))].map((name) => ({ name })));
      ...
      getLastEventFlow().then((res) => {
        if (res.data.code == 0) {
          ...
          let uniqueDeptNames = res.data.data
            .filter((item) => !item.performanceIsCustom)
            .map((item) => ({
              deptName: item.deptName,
          ...
          this.deptNameList = this.deptNameList.map((item) => {
            ...
            if (foundItem) {
              return {
                deptName: item.deptName,
                performanceFlowId: foundItem.performanceFlowId,
              };
            } else {
              return {
                deptName: item.deptName,
                performanceFlowId: null,
              };
            }
          });
    ...
    selectPerformance() {
      ...
      //   console.log(this.deptNameList, deptNameList);
    },
    handelClose() {
    ...
    //选择审批名称后找对应的审批人
    getSelectedUsers(selectedId) {
      const selectedRow = this.transformed.find((item) => item.performanceFlowId === selectedId);
    ...
    //删除事件，回退到未发布的状态
    backEvent(row) {
      ElMessageBox.confirm("是否确定回退该事件，回退将删除所有信息", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        backEvent(row.eventId, this);
      });
    },
  },
};
