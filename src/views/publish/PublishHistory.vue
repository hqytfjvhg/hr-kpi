<template>
  <div class="publishHistory">
    ...
    <el-form class="titleForm">
      ...
      <el-button @click="getEventHistory" type="primary">{{ $t("inquire") }}</el-button>
    </el-form>

    <el-table
      border
      :data="eventHistoryTableData"
      @expand-change="selectEvent"
      :height="tableHeight"
          ...
          <div class="expand-style">
            <div class="card-style">
              <el-card shadow="never">
                    ...
                    <el-form>
                      <el-form-item label="部门">
                        <el-select
                          clearable
                          placeholder="请选择"
                          v-model="deptName[scope.row.eventId]"
                          @change="selectValueByDept(scope.row)"
                        >
                          <el-option
                            v-for="item in deptOptions[scope.row.eventId]"
                            :key="item.deptName"
                            :label="item.deptName"
                            :value="item.deptName"
                          />
                      ...
                      <!-- <el-button style="margin-left: 10px" @click="selectValueByDept(scope.row)" type="primary"
                        >筛选</el-button
                      > -->
                    </el-form>
                  </div>
                ...
                <div class="scroll">
                  <el-table v-if="joinUserList[scope.row.eventId]" :data="joinUserListByDept[scope.row.eventId]">
                    <el-table-column label="姓名" prop="name"></el-table-column>
                    <el-table-column label="状态">
                      <template #default="scope">
                        <span v-if="scope.row.activityState" style="color: #f56c6c">被移除</span>
                        <span v-else>
                          <span style="color: #67c23a" v-if="scope.row.complete">已完成</span>
              ...
              <el-card shadow="never">
                ...
                <div class="scroll">
                  <el-table
                    ...
                    :data="targetJoinUserListByDept[scope.row.eventId]"
                  >
                    <el-table-column label="姓名" prop="name"></el-table-column>
                    <el-table-column label="状态">
                      <template #default="scope">
                        <span style="color: #f56c6c" v-if="scope.row.activityState">被移除</span>
                        <span v-else>
                          <span style="color: #67c23a" v-if="scope.row.complete">已完成</span>
                          <span style="color: #e6a23c" v-else>未完成</span>
                        </span>
...
<script>
import { getEventHistory, getEventJoinUser } from "@/api/values/index";
import { ElMessage } from "element-plus";
import { getTargetJoinPeople } from "@/api/performance/index";
// import store from "@/store";
// import { SuccessFilled, MoreFilled, Close } from "@element-plus/icons-vue";
import PublishHistoryValue from "./PublishHistoryValue.vue";
import PublishHistoryTarget from "./PublishHistoryTarget.vue";
export default {
  name: "publishHistory",
  components: { PublishHistoryValue, PublishHistoryTarget },
  data() {
    return {
      activeName: "history",
      ...
      deptOptions: {}, //筛选部门
      deptName: {},
      publishHistoryValueRow: null, //传输给进度弹窗的数据
      publishHistoryTargetRow: null,
    };
  },
  mounted() {
  ...
  methods: {
          ...
          .then((res) => {
            if (res.data && res.data.code == 0) {
              ...
              let result = tableData.reduce((acc, curr) => {
                //按事件筛选部门下拉框
                if (!this.deptOptions[curr.eventId]) {
                  this.deptOptions[curr.eventId] = [];
                }
                let index = this.deptOptions[curr.eventId]?.findIndex((item) => item.deptName == curr.deptName);
                if (index == -1) {
                  this.deptOptions[curr.eventId].push(curr);
                }
                //参与价值观的人
                if (!acc[curr.eventId]) {
                  acc[curr.eventId] = {
                ...
                acc[curr.eventId].list.push({
                  ...
                  activityState: curr.activityState,
                });
                return acc;
              }, {});
              ...
              this.joinUserListByDept[row.eventId] = this.joinUserList[row.eventId].list;
        ...
        getTargetJoinPeople(sendData)
          .then((res) => {
            if (res.data.code == 0) {
              ...
              this.targetJoinUserListByDept[row.eventId] = this.targetJoinUserList[row.eventId].list;
            }
          })
          .catch((err) => {
            console.log(err);
    ...
    selectCurrentPerformance(row) {
      ...
      this.publishHistoryTargetRow = row;
      // const data = {
      //   year: this.year,
      //   month: this.month,
      //   userId: row.userId,
    ...
    //筛选
    selectValueByDept(row) {
      if (this.deptName[row.eventId]) {
        this.targetJoinUserListByDept[row.eventId] = this.targetJoinUserList[row.eventId].list.filter(
          (item) => item.deptName == this.deptName[row.eventId],
        );
        this.joinUserListByDept[row.eventId] = this.joinUserList[row.eventId].list.filter(
          (item) => item.deptName == this.deptName[row.eventId],
        );
      } else {
        this.targetJoinUserListByDept[row.eventId] = this.targetJoinUserList[row.eventId].list;
