<template>
  <div class="commonApproval">
    <div class="button-group">
      <el-tooltip content="新增审批流程" placement="top" effe ct="light" v-if="!isRootRead && !isRootDown">
      ...
      <el-table :data="transformed" border :height="tableHeight" style="width: 100%; text-align: center" type="index">
        ...
        <el-table-column label="流程名称">
          <template #default="scope">
            ...
            <el-input v-else v-model="scope.row.hrFlowName" placeholder="请输入流程名称" clearable></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" v-if="!isRootRead && !isRootDown">
          <template #default="scope">
            <div class="operateStyle">
              ...
              <div @click="deleteNull(scope.row)" v-if="scope.row.hrFlowId == null" style="margin-left: 10px">
...
export default {
  ...
  computed: {
    filteredData() {
      return this.transformed.map((item) => {
        const userNames = Object.fromEntries(Object.entries(item).filter(([key]) => key.startsWith("hrUserName")));
        return userNames;
      });
    },
  },
  ...
  methods: {
    ...
    deleteNull(val) {
      const index = this.transformed.findIndex((item) => item.hrFlowName === val.hrFlowName);
