    ...
    <div class="button-group">
      <div class="form-style">
        <el-form class="titleForm">
          ...
          <el-button @click="getUserList" type="primary">{{ $t("refresh") }}</el-button>
        </el-form>
      </div>

      <el-button type="success" @click="addUser" v-if="!isRootRead && !isRootDown"
        ><el-icon class="iconStyle"><Plus /></el-icon>{{ $t("add") }}</el-button
      >
      <el-button type="primary" @click="downloadUserTemplate" v-if="!isRootRead"
        ><el-icon class="iconStyle"><Download /></el-icon>{{ $t("templet") }}</el-button
      >
      ...
      >
        <el-button type="primary"
          ><el-icon class="iconStyle"><Download /></el-icon>{{ $t("import") }}</el-button
        >
      </el-upload>

      <el-button type="warning" @click="exportUserInfo" v-if="!isRootRead && !isRootDown"
        ><el-icon class="iconStyle"><Upload /></el-icon>{{ $t("export") }}</el-button
      >
    </div>

    <el-table :data="userListData" :height="tableHeight" border ref="table">
      ...
      <el-table-column align="center">
        ...
        <template #default="{ row }">
          <el-switch
            v-model="row.customFlow"
            inline-prompt
            active-text="是"
            inactive-text="否"
            :disabled="isRootRead || isRootDown"
            @change="changeUserValueCustom(row)"
            v-if="row.roleName !== 'ROOT' && row.roleName !== 'ROOT-DOWNLOAD' && row.roleName !== 'ROOT-READ'"
        /></template>
      </el-table-column>
      <el-table-column align="center">
        <template #header
        ...
        <template #default="{ row }">
          <el-switch
            v-model="row.targetCustomFlowState"
            inline-prompt
            active-text="是"
            inactive-text="否"
            @change="changeUserPerformCustom(row)"
            :disabled="isRootRead || isRootDown"
            v-if="row.roleName !== 'ROOT' && row.roleName !== 'ROOT-DOWNLOAD' && row.roleName !== 'ROOT-READ'"
        /></template>
      </el-table-column>
      <el-table-column label="操作" width="150" align="center" v-if="!isRootRead && !isRootDown">
        <template #default="{ row }">
          <el-tooltip
            effect="light"
            :content="row.roleName == 'ROOT' ? '管理员账户不可删除' : '删除用户'"
            placement="top"
            :show-after="1000"
          >
            <el-button type="info" size="small" circle v-if="row.roleName == 'ROOT'"
              ><el-icon><Delete /></el-icon
            ></el-button>
            <el-button type="danger" size="small" circle @click="deleteRow(row)" v-else
              ><el-icon><Delete /></el-icon
            ></el-button>
          </el-tooltip>
          ...
          <el-tooltip
            effect="light"
            :content="row.roleName == 'ROOT' ? '管理员账户不可修改' : '修改用户信息'"
            placement="top"
            :show-after="1000"
          >
            <el-button type="info" size="small" circle v-if="row.roleName == 'ROOT'"
            ...
            ></el-button>
            <el-button type="primary" size="small" circle @click="reviseVal(row)" v-else
              ><el-icon><Edit /></el-icon
            ></el-button>
          </el-tooltip>
          ...
          <el-tooltip
            effect="light"
            :content="row.roleName == 'ROOT' ? '管理员账户不可重置密码' : '重置用户密码'"
            placement="top"
            :show-after="1000"
          >
            <el-button type="info" size="small" circle v-if="row.roleName == 'ROOT'"
            ...
            ></el-button>
            <el-button type="primary" size="small" circle @click="changeRow(row)" v-else
              ><el-icon><Refresh /></el-icon
            ></el-button>
          </el-tooltip>
        </template>
...
import {
  ...
  exportUserInfo,
  batchAddUser,
  changeUserValueCustom,
  changeUserPerformCustom,
...
export default {
  name: "UserList",
  components: { Dialog, Plus, Edit, Delete, Download, Upload, Refresh },
  setup() {
    const data = reactive({
      name: null,
      role: null,
    ...
    onMounted(async () => {
      ...
      data.isRootRead = rootPermission.isRootRead();
      data.isRootDown = rootPermission.isRootDown();
    });
    const method = reactive({
        ...
        userList(userdata)
          .then((res) => {
            ...
            data.userListData.map((item) => {
              item.clerkAssociatedDepartmenString = JSON.parse(item.clerkAssociatedDepartmenString);
              item.clerkAssociatedUsersString = JSON.parse(item.clerkAssociatedUsersString);
              // console.log(JSON.parse(item.clerkAssociatedUsersString));
              // item.clerkAssociatedUsersString = item.clerkAssociatedUsersString.map(String);
            });
      ...
      deleteRow(val) {
        ElMessageBox.confirm("确定删除此员工吗?", "提示", {
      ...
      async reviseRow(val) {
        ...
        const reviseData = {
          ...
          clerkAssociatedDepartmentList: val.clerkAssociatedDepartmenString,
          clerkAssociatedUserIdList: val.clerkAssociatedUsersString,
          //不做解析的话：修改弹窗绑定的字段就行
          //  roleId: val.roleId,
          // deptId: val.deptId,
        };
      ...
      changeUserValueCustom(row) {
        changeUserValueCustom({ customFlowState: row.customFlow, userId: row.userId });
      },
      changeUserPerformCustom(row) {
        changeUserPerformCustom({ customFlowState: row.targetCustomFlowState, userId: row.userId });
      },
    });
