  ...
  <div class="KpiScoreTable">
    <el-form class="titleForm">
      <el-form-item :label="$t('year')" prop="year">
        <el-select :placeholder="$t('choose')" v-model="year" style="min-width: 50px" @change="getKPIScore()">
          <el-option v-for="item in yearOptions" :key="item" :label="item" :value="item" />
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('month')" prop="month">
        <el-select :placeholder="$t('choose')" v-model="month" style="min-width: 50px" @change="getKPIScore()">
      ...
      <el-form-item :label="$t('userName')"
        ><el-input :placeholder="$t('input')" v-model="name" style="min-width: 60px"></el-input
      ></el-form-item>

      <el-form-item :label="$t('state')">
        <el-select clearable :placeholder="$t('choose')" v-model="finishState" style="min-width: 60px">
          <el-option
          ...
          ></el-option></el-select
      ></el-form-item>
      <el-form-item :label="$t('bouns') + '≥120%'">
        <el-select clearable :placeholder="$t('choose')" v-model="kpiScoreState" style="min-width: 60px">
          <el-option v-for="item in kpiScoreOptions" :key="item.name" :label="item.name" :value="item.state"></el-option
        ></el-select>
      </el-form-item>
      <el-button @click="getKPIScore" type="primary">{{ $t("inquire") }}</el-button>
    ...
    <el-skeleton animated :loading="loading" :rows="5" style="text-align: left">
      <el-table :data="tableData1" border :height="tableHeight" id="kpi-table">
        ...
        <el-table-column prop="valueScore">
          <template #header>
            <div>{{ $t("valueScore") }}<br />{{ $t("score") }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="valueRealScore">
          <template #header>
            <div style="min-width: 60px">{{ $t("valueScore") }}<br />{{ $t("realScore") }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('target')" min-width="200">
        ...
        <el-table-column>
          <template #header>
            <div style="min-width: 60px">{{ $t("target") }}<br />{{ $t("score") }}</div>
          </template>
          <template #default="scope">
            <div v-for="item in scope.row.targetInfoList" :key="item">
        ...
        <el-table-column prop="targetScore">
          <template #header>
            <div style="min-width: 60px">{{ $t("target") }}<br />{{ $t("realScore") }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('bouns')" prop="bonusCoefficient" min-width="60">
          <template #default="scope">{{ scope.row.bonusCoefficient }}%</template>
        </el-table-column>

        <el-table-column :label="$t('valueScore')" prop="complete" v-if="isShowComplete">
          <template #default="scope">
            <span
              style="color: #f56c6c"
        ...
        <el-table-column :label="$t('performanceScore')" v-if="isShowComplete">
          <template #default="scope">
            <span
              style="color: #e6a23c"
    ...
    <div style="text-align: right; margin-top: 10px" v-if="!isRootRead">
      <el-button type="primary" @click="downLoadKPIExcel"
        >{{ $t("export") }}{{ this.month }}{{ $t("month") }}{{ $t("downBouns") }}</el-button
      >
      <el-button type="primary" @click="downLoadOneYearKPIExcel"
        >{{ $t("export") }}{{ this.year }}{{ $t("downPerformance") }}</el-button
      >
    </div>
    ...
    <el-dialog v-model="isRemove" title="警告" style="width: 500px" align-center>
      ...
      <div style="text-align: right; margin-top: 20px">
        <el-button @click="isRemove = false">{{ $t("cancle") }}</el-button
...
export default {
  ...
  data() {
    return {
      ...
      tableHeight: null,
      finishStateOptions: [
        { name: i18n.global.t("noWrite"), state: "noWrite" },
        { name: i18n.global.t("finish"), state: "finish" },
        { name: i18n.global.t("noFinish"), state: "noFinish" },
      ], //状态的选择
      kpiScoreOptions: [{ name: i18n.global.t("yes"), state: true }],
      loading: true, //骨架屏加载
