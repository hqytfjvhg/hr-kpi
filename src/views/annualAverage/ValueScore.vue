    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>
            <el-select v-model="ValueYear" class="bonusSelectStyle" @change="getValueAllYear" style="width: 90px">
              <el-option v-for="item in $store.state.yearOptions" :key="item" :label="item" :value="item" /> </el-select
            >年
            <el-select v-model="ValueMonth" @change="getValueOneMonth" style="width: 90px">
              <el-option
      ...
      <div id="EChartValueOneMonth" style="width: 100%; height: 310px"></div>
    </el-card>
    <el-card shadow="never" style="margin-top: 20px">
      <template #header>
        <div class="card-header">
          <span>{{ ValueYear }}年各部门价值观得分</span>
        </div>
      </template>
      <div id="EChartValueAllYear" style="width: 100%; height: 310px"></div>
    </el-card>
  </div>
</template>
...
import { getValueAllYear } from "@/api/annualAverage";
...
export default {
  ...
  mounted() {
    this.ValueYear = store.state.year;

    if (JSON.parse(store.state.eventSave)) {
      this.ValueMonth = store.state.eventMonth;
    } else {
      this.ValueMonth = Number(store.state.eventMonth) - 1;
      if (this.ValueMonth == 0) {
        this.ValueMonth = 12;
        this.ValueYear = this.ValueYear - 1;
      }
    }
    this.getValueAllYear();
  },
  methods: {
    getValueAllYear() {
      getValueAllYear({ year: this.ValueYear }).then((res) => {
        if (res.data.code == 0) {
          ...
          this.getValueOneMonth();
        }
      });
    },
    getValueOneMonth() {
      this.yAxisDataOneMonth = this.departmentsAllYear
        .filter((item) => item.month == this.ValueMonth)
    ...
    getEchartOneMonth() {
      let EChartOneMonth = echarts.init(document.getElementById("EChartValueOneMonth"));
    ...
    getEchartAllYear() {
      let EChartAllYear = echarts.init(document.getElementById("EChartValueAllYear"));
      EChartAllYear.clear();
      //控制文本的字段
      const labelOption = {
        show: true,
