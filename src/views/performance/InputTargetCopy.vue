        ...
        <el-table-column prop="targetName">
          ...
          <el-table-column label="实现值" prop="actualAchievementRate" align="center" min-width="100">
            <template #default="scope">
              ...
              <div v-if="scope.row.targetInfoList[index].maxRate" class="tooltip">
                范围：
  ...
  methods: {
    ...
    addDecimal(val, index, rowIndex) {
      if (Number.isInteger(Number(val)) && !val.includes(".")) {
        ...
        // 新增判断：如果是小数且小数位只有一位，则在后面添加 "0"
        this.oneDeptPerformanceInfo[rowIndex].targetInfoList[index].actualAchievementRate = `${val}0`;
      }
    },
    //提交数据
    sendPerformanceData() {
