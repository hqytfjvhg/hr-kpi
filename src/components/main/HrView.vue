<template>
  <div>
    ...
    <div style="display: flex; justify-content: space-between">
      <div style="width: 70%">
        <div style="border-radius: 10px; background-color: #fff; display: flex; width: 100%; border: 1px solid #ebeef5">
          <div style="width: 50%">
            <div
              style="
        ...
        <el-card style="text-align: left; border-radius: 10px; margin-top: 15px" shadow="never">
          <template #header>
            <div class="card-header">
              <span>未填写价值观名单</span>
              <span>
                <span style="color: #909399; font-size: small" v-if="noWriteValueList && noWriteValueList.valueList"
                  >{{ noWriteValueList.valueList.length }}人未填</span
                >
                <span style="margin-left: 8px; color: #e6a23c">
                  <el-tooltip effect="light" placement="top" content="刷新">
                    <i
                      class="fa fa-repeat"
                      aria-hidden="true"
                      @click="getNoWriteValueListAndNoApprovalDept" /></el-tooltip></span
              ></span>
            </div>
          </template>
          <div>
            <el-table :data="noWriteValueList.valueList" height="150px">
        ...
        <el-card style="text-align: left; border-radius: 10px; margin-top: 15px" shadow="never">
          <template #header>
            <div class="card-header">
              <span>未完成指标录入名单</span>
              <span>
                <span style="color: #909399; font-size: small" v-if="noWriteValueList && noWriteValueList.deptNameList"
                  >{{ noWriteValueList.deptNameList.length }}人未录</span
                >
                <span style="margin-left: 8px; color: #e6a23c">
                  <el-tooltip effect="light" placement="top" content="刷新">
                    <i
                      class="fa fa-repeat"
                      aria-hidden="true"
                      @click="getNoWriteValueListAndNoApprovalDept" /></el-tooltip
                ></span>
              </span>
            </div>
          </template>
          <div>
            <el-table :data="noWriteValueList.deptNameList" height="150px">
              <el-table-column label="部门" prop="deptName"></el-table-column>
...
import {
  ...
  getOneselfAllYear,
  getNoWriteValueListAndNoApprovalDept,
...
export default {
  ...
  async mounted() {
    ...
    this.getNoWriteValueListAndNoApprovalDept();
  },
  methods: {
    //获取整年KPI
    getAllYearKPI(data) {
    ...
    //人事查看价值观未填写的名单
    getNoWriteValueListAndNoApprovalDept() {
      getNoWriteValueListAndNoApprovalDept().then((res) => {
        if (res.data.code == 0) {
