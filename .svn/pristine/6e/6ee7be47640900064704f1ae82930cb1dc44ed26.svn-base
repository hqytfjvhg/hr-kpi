    ...
    <div class="writeTableStyle">
      ...
      >
        <tr style="height: 160px; text-align: center" v-if="index == 0">
          <td colspan="2">
            <div style="font-weight: bolder; font-size: 1.5rem; padding-bottom: 15px; color: #698db5">
              <span v-if="$store.state.role == 'ROOT'">【{{ $store.state.currentName }}】</span>
              <span v-else>【{{ $store.state.name }}】</span> {{ item.month }}月价值观
            </div>
      ...
      <div style="text-align: right; min-width: 700px; background-color: #fff">
  ...
  methods: {
    ...
    sendValuesForm() {
      try {
        ...
        this.$confirm(`自评得分: 【${selfTotalScore}分】`, "提示", {
