    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>
            <el-select v-model="KPIYear" @change="getKPIAllYear" style="width: 90px">
              <el-option v-for="item in $store.state.yearOptions" :key="item" :label="item" :value="item" />
            </el-select>
            年
            <el-select v-model="KPIMonth" @change="getKPIOneMonth" style="width: 90px">
              <el-option
      ...
      <div id="EChartKPIOneMonth" style="width: 100%; height: 310px"></div>
    </el-card>
    <el-card shadow="never" style="margin-top: 20px">
      <template #header>
        <div class="card-header">
          <span>{{ KPIYear }}年各部门全年KPI得分</span>
        </div>
      </template>
      <div id="EChartKPIAllYear" style="width: 100%; height: 310px"></div>
    </el-card>
  </div>
</template>
...
import { getKPIAllYear } from "@/api/annualAverage";
...
export default {
  ...
  async mounted() {
    this.KPIYear = store.state.year;
    if (JSON.parse(store.state.eventSave)) {
      this.KPIMonth = store.state.eventMonth;
    } else {
      this.KPIMonth = Number(store.state.eventMonth) - 1;
      if (this.KPIMonth == 0) {
        this.KPIMonth = 12;
        this.KPIYear = this.KPIYear - 1;
      }
    }
    this.getKPIAllYear();
  },
  methods: {
    getKPIAllYear() {
      getKPIAllYear({ year: this.KPIYear }).then((res) => {
        if (res.data.code == 0) {
          ...
          this.yAxisDataAllYear = result.map((item, index) => {
            ...
            return Math.round((sum[index] / result[index].data.length) * 100) / 100;
          });
    ...
    getKPIOneMonth() {
      this.yAxisDataOneMonth = this.departmentsAllYear
        .filter((item) => item.month == this.KPIMonth)
    ...
    getEchartOneMonth() {
      let EChartOneMonth = echarts.init(document.getElementById("EChartKPIOneMonth"));
    ...
    getEchartAllYear() {
      let EChartAllYear = echarts.init(document.getElementById("EChartKPIAllYear"));
      EChartAllYear.clear();
      //控制文本的字段
      const labelOption = {
        show: true,
