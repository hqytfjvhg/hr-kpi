<template>
  <div class="performance">
    ...
    <div class="performance-title">
      正在审批【{{ $store.state.currentName }}】{{ $store.state.criticalMonth }}月份的业绩审批
    </div>
    <el-table
      :data="performanceList"
      :header-cell-style="{ fontSize: '15px', color: '#606260', fontWeight: 'normal' }"
      stripe
      border
    >
      ...
      <el-table-column label="审批结果" prop="actualAchievementRate" v-if="state == true">
        <template #default="scope">
          ...
          if (res.data.data[0].actualAchievementRate !== "" && res.data.data[0].actualAchievementRate !== null) {
            res.data.data.map((item) => {
              ...
              const result = Object.keys(actual).map((key) => {
                ...
                item.currentActualAchievementRate = value;
        ...
        if (this.uploadFile == null) {
          ...
          if (data.every((item) => item != undefined)) {
            ElMessageBox.confirm("确定不提交附件吗？", "警告", {
              ...
              .then(() => {
                axios.post("/ifi-personal/performance/upadateUserTargetDataNoFile", data).then((res) => {
                  if (res.data.code == 0) {
                    // console.log(true);
                    ElMessage.success("提交成功");
                    router.replace({ name: "aboutInfo", query: { activeName: "second" } });
          ...
          if (this.formData != null) {
            ...
            ElMessageBox.confirm("确定提交吗？", "警告", {
              ...
              .then(() => {
                axios.post("/ifi-personal/performance/upadateUserTargetData", this.formData).then((res) => {
                  if (res.data.code == 0) {
                    // console.log(true);
                    ElMessage.success("提交成功");
