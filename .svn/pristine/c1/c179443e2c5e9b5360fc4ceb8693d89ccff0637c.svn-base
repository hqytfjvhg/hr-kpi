<template>
  <div class="performanceTable">
    <el-form class="formStyle">
      ...
      <el-button @click="getperformanceDept" type="primary">{{ $t("refresh") }}</el-button>
    </el-form>
        ...
        <template #default="scope">
          ...
          >
            ...
            <div v-for="(remark, index) in scope.row.explain" :key="index">
              <span v-if="remark.split('：')[1] !== '未填写'" style="font-size: 14px"
                >{{ remark }}<br v-if="index !== scope.row.explain.length - 1"
              /></span>
            </div>
          </el-popover>
        </template>
